			...
			if(recipe[i]==0){
				...
				if(targ->size_dep!=0){
					...
					for(size_t k=0;k<nb_dep;k++){
						...
						for(size_t j=0;j<strlen(targ->dependencies[k]);j++){
...
int exec_target_and_dep(struct makefile* make,struct target* targ){
	for(size_t i=0;i<targ->size_dep;i++){
		struct target* targ_d=get_target_with_name(make,targ->dependencies[i]);
		if(targ_d==NULL){
			continue;
		}
		if((nothing_to_be_done(make,targ_d)||is_up_to_date(make,targ_d)) && !is_in_list_phony(make,targ_d)){
			continue;
		}
...
static struct target* new_target_with_steam(struct target* targ_pattern,char*stem){
	struct target* new_targ=init_target(str_with_stem(targ_pattern->name,stem));

	for(size_t i=0;i<targ_pattern->size_dep;i++){
		add_target_dependencies(new_targ,str_with_stem(targ_pattern->dependencies[i],stem));
	}
...
static struct target* looking_for_pattern(struct makefile* make,char* name_target){// creer une target en remplacer le stem dams target et dependencies
	size_t min=SIZE_MAX;
	char* stem_res=NULL;
	...
	for(size_t i=0;i<make->list_t->size;i++){
		...
		if(targ->is_pattern){
			char* stem =stem_if_valid(targ->name,name_target);// stem pas vide 
			if(stem){
				if(strlen(stem)<min){
					min=strlen(stem);
					stem_res=stem;
