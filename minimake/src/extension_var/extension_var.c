...
static char *get_name_rec(struct makefile *make, char *str, int *ptr_i,
                          char last);
// vu que recursion boucle

static void apres_dollars(struct makefile *make, char *str, int *ptr_i,
                          struct string *str_name)
{
    *ptr_i = *ptr_i + 1;
    char *name_int = NULL;
                          ...
                          char last)
{
    ...
    while (str[*ptr_i] && str[*ptr_i] != last)
    { // str est valide donc pas possible d arrive a '\0' si last!='\0'
        if (str[*ptr_i] != '$')
        {
