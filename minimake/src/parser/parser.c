...
static void remove_end(char* line){ //return la new len
	if(!line) return ;
...
static void parser_add_dependencies(struct makefile* make,struct target* targ,char* line){
...
int parser(FILE* file,struct makefile* make){
	...
	while(getline(&line,&len,file)!=-1){
		...
		if(type==PHONY){
			if(parser_phony(make,line)==2 || parser_target(file,make,line)==2){
